...
class CustomCLIP(nn.Module):
    ...
    def forward(self, xs):
        ...
        onedomains, twodomains = [],[]
        for i in range(self.num_domains):
            d1 = F.log_softmax(self.domains[i](p_relus[i]), dim=1)
            #y = y.view(sh_relu[i].shape[0],-1)
            #d2 = F.log_softmax(self.domains[i](n_relus[i]), dim=1)
            onedomains.append(d1)
            #onedomains.append(d2)
        '''
        for i in range(self.num_domains):
            for j in range(self.num_domains):
                if(j!=i):
                    twodomains.append(F.log_softmax(self.domains[i](p_relus[j]), dim=1))
                    twodomains.append(F.log_softmax(self.domains[i](n_relus[j]), dim=1))
        '''
        ...
        return logits, nlogits, onedomains, pull_domains, push_domains
...
class CoOp(TrainerX):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            loss_summary = self.forward_backward(batches, val_batch)
    ...
    def forward_backward(self, batches, val_batch):
        device = "cuda" 
        #gamma = 10.0
        #coeff0 = 0.01
        #coeff1 = 1
        ...
        source_labels = torch.ones(b, requires_grad=False).type(torch.LongTensor).to(device)
        ...
        target_labels = torch.zeros(b, requires_grad=False).type(torch.LongTensor).to(device)
        #for j in range(self.num_domains):
        ...
        logits, nlogits, onedomains, pull_domains, push_domains = self.model(xs, xt)

        domain_loss1 = 0
        for i in range(n):
            domain_loss1 += F.nll_loss(onedomains[i], source_labels)
        #for i in range(2*n*(n-1)):
            #domain_loss1 += F.nll_loss(twodomains[i],zerolabels)
        domain_loss2 = 0
        for i in range(n*(n-1)):
            domain_loss2 += F.nll_loss(pull_domains[i], target_labels)
        domain_loss3 = 0
        for i in range(n*(n-1)):
            domain_loss3 += F.nll_loss(push_domains[i],push_labels)
        ...
        #losses_avg = losses / n
        losses = class_loss1  + 0.01 * class_loss2 #+ 0.02 * ( domain_loss1/(n) + domain_loss2/n/(n-1) +  domain_loss3/n/(n-1) )
