...
class CustomCLIP(nn.Module):
    ...
    def forward(self, xs):
        ...
        return logits, onedomains, pull_domains, push_domains
...
class CoOp(TrainerX):
    ...
    def forward_backward(self, batches):
        ...
        onelabels = torch.ones(b, requires_grad=False).type(torch.LongTensor).to(device)
        ...
        push_labels = torch.zeros(b, requires_grad=False).type(torch.LongTensor).to(device)
        #for j in range(self.num_domains):
        ...
        logits, onedomains, pull_domains, push_domains = self.model(xs)

        domain_loss1, domain_loss2 = 0, 0
        for i in range(2*n):
            domain_loss1 += F.nll_loss(onedomains[i],onelabels)
        #for i in range(2*n*(n-1)):
            #domain_loss1 += F.nll_loss(twodomains[i],zerolabels)
        for i in range(n*(n-1)):
            domain_loss2 += F.nll_loss(pull_domains[i],pull_labels)
        for i in range(n*(n-1)):
            domain_loss2 += F.nll_loss(push_domains[i],push_labels)
        ...
        class_loss = 0
        for j in range(n):
            class_loss += F.cross_entropy(logits[j], ys[j])
            #domain_losses = torch.stack([F.nll_loss(sdomains[j], slabels) for j in range(self.num_domains)])
            #domain_losses = torch.stack([F.nll_loss(sdomains[j], slabels) +
                        #F.nll_loss(nsdomains[j][0], nslabels) + F.nll_loss(nsdomains[j][1], nslabels) for j in range(self.num_domains)])

        #losses_avg = losses / n
        losses = class_loss + coeff1 * domain_loss1 + coeff2 * domain_loss2
