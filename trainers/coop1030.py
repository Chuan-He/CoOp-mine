...
class CustomCLIP(nn.Module):
    ...
    def forward(self, xs, xt):
        ...
        timage_feature = self.image_encoder(xt)
        '''
        for i in range(self.num_domains):
        ...
        timage_feature = timage_feature / timage_feature.norm(dim=-1, keepdim=True)
        ...
        sh_relus = []
        for i in range(self.num_domains):
            sh_relu = image_features[i]
            for hidden in self.hiddens:
                sh_relu = F.relu(hidden(sh_relu))
            sh_relus.append(sh_relu)
        
        t_relu = timage_feature
        for hidden in self.hiddens:
            t_relu = F.relu(hidden(t_relu))
        ...
        for i in range(self.num_domains):
            d1 = F.log_softmax(self.domains[i](self.grls[i].apply(sh_relus[i])), dim=1)
            #y = y.view(sh_relu[i].shape[0],-1)
            d2 = F.log_softmax(self.domains[i](self.grls[i].apply(t_relu)), dim=1)
            source_domains.append(d1)
            target_domains.append(d2)


        return logits, source_domains, target_domains
...
class CoOp(TrainerX):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx in range(self.num_batches):
            ...
            loss_summary = self.forward_backward(batches, test_batch)
    ...
    def before_epoch(self):
        super().before_epoch()
        print(f"{self.model.prompt_learner.ctx}")
        #print(f"{self.model.feature_selector.stochastic_gate}")

    def forward_backward(self, batches, test_batch):
        device = "cuda" 
        #gamma = 10.0
        #coeff0 = 0.01
        #coeff1 = 1
        ...
        xt = test_batch['img'].to(device)
        #yt = val_batch['label'].to(device)

        # Training phase.
        
        one_labels = torch.ones(b, requires_grad=False).type(torch.LongTensor).to(device)
        ...
        zero_labels = torch.zeros(b, requires_grad=False).type(torch.LongTensor).to(device)
        ...
        logits, s_domains, t_domains = self.model(xs, xt)
        losses = torch.stack([F.cross_entropy(logits[j], ys[j]) for j in range(n)])
                
        domain_losses = torch.stack([F.nll_loss(s_domains[j], one_labels) +
                                           F.nll_loss(t_domains[j], zero_labels) for j in range(n)])

        # Different final loss function depending on different training modes.
        if mode == "maxmin":
            loss = torch.max(losses) + 0.01 * torch.min(domain_losses)
        elif mode == "dynamic":
            loss = torch.log(torch.sum(torch.exp(10.0 * (losses + 0.01 * domain_losses)))) / 10.0
        else:
            #raise ValueError("No support for the training mode on madnNet: {}.".format(mode))
            loss = torch.min(losses + 0.01 * domain_losses)
    ...
    def build_model(self):
        ...
        #self.model.hiddens.requires_grad_(True)
        #self.model.domains.requires_grad_(True)
        self.model.prompt_learner.requires_grad_(True)
        for name, param in self.model.named_parameters():
